...
const ResultView: React.FC<ResultViewProps> = ({ answers, timeTaken, onTabChange }) => {
  ...
  // ... (rest of the logic remains the same until the buttons)
  // Stats calculation
  ...
  const correctCount = mcTasks.reduce((acc, task) => {
    return acc + (answers[task.id] === correctAnswers[task.id] ? 1 : 0);
  }, 0);
        ...
        <div className="p-5 md:p-6">
          ...
          <div className="space-y-4">
            ...
            {quizData.map((section) => {
              ...
              const sectionTasks = section.tasks;
              const sectionMCQs = sectionTasks.filter(t => t.type === "Multiple Choice");
              const sectionCorrectCount = sectionMCQs.reduce((acc, task) => {
                return acc + (answers[task.id] === correctAnswers[task.id] ? 1 : 0);
              }, 0);
              const sectionScore = sectionMCQs.length > 0
                ? Math.round((sectionCorrectCount / sectionMCQs.length) * 100)
                : 100;

              let statusLabel = "Needs Focus";
              let statusColor = "text-red-500";
              let statusBgLabel = "text-red-400";
              
              if (sectionScore >= 75) {
                statusLabel = "Strong";
                statusColor = "text-emerald-500";
                statusBgLabel = "text-emerald-400";
              } else if (sectionScore >= 50) {
                statusLabel = "Moderate";
                statusColor = "text-amber-500";
                statusBgLabel = "text-amber-400";
              }
                ...
                <div key={section.id} className="bg-white rounded-[20px] border border-slate-100 overflow-hidden shadow-xs">
                  <div className="px-5 py-4 border-b border-slate-50 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <span className={cn("text-[14px] font-semibold font-poppins leading-none", statusColor)}>{sectionScore}%</span>
                      <p className={cn("text-[9px] font-bold font-poppins uppercase mt-1", statusBgLabel)}>{statusLabel}</p>
                    </div>
                  </div>
