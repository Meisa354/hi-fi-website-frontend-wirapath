...
const QuizView: React.FC<QuizViewProps> = ({ onComplete, onBack }) => {
  ...
  const handleNextSection = () => {
    if (activeSectionIdx < quizData.length - 1) {
      ...
      // 1. Format answers untuk backend
      const formattedAnswers = Object.entries(answers).map(([qId, oId]) => ({
        questionId: qId,
        optionId: oId
      }));
      
      // 2. HITUNG SKOR DINAMIS BERDASARKAN KUNCI JAWABAN YANG BENAR
      ...
      const mcTasks = quizData.flatMap(s => s.tasks).filter(t => t.type === "Multiple Choice");
      const correctCount = mcTasks.reduce((acc, task) => {
        return acc + (answers[task.id] === correctAnswers[task.id] ? 1 : 0);
      }, 0);

      const calculatedScore = mcTasks.length > 0 
        ? Math.round((correctCount / mcTasks.length) * 100) 
        : 0;
      ...
      fetch(`${API_BASE_URL}/api/quizzes/submit`, {
        ...
        body: JSON.stringify({
          ...
          scoreFromFrontend: calculatedScore
        })
      })
      .then(res => res.json())
      .then(data => {
