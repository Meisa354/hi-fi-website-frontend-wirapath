...
export const ChatSimulation = ({
      ...
      ? [
          {
            id: "1",
            sender: "bot",
            text: `Welcome to your interview simulation at ${companyName} for the ${jobRole} role. I'll be acting as your technical recruiter today. Let's begin!`,
          },
          {
            id: "2",
            sender: "bot",
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/simulations/start`, {
      ...
      body: JSON.stringify({ 
        userId: 1,
        jobPositionId: type === 'recruiter' ? jobPositionId : null,
        simulationType: simTypeMap[type] || 'Recruiter'
      })
    })
    .then(res => res.json())
    .then(data => {
      if (data.success) {
        ...
        .then(detailData => {
          if (detailData.success && detailData.data.quickReplies) {
            setQuickReplies(detailData.data.quickReplies);
          }
        })
  ...
  }, [jobPositionId, type, companyName]);
    ...
    if (sessionId) {
      ...
      .then(data => {
        if (data.success && data.data.length >= 2) {
          const botChat = data.data[1]; // Bot chat is second element from API response
          setMessages((prev) => [
            ...prev,
            { id: botChat.id.toString(), sender: 'bot', text: botChat.message }
          ]);
        }
      })
      .catch(err => console.error("Error sending message:", err));
    } else {
      // Fallback if API fails/not loaded
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
      ...
      <div className="px-4 py-4 bg-white border-t border-[#F1F5F9] shrink-0">
        ...
        {showSuggestions && type === "recruiter" && (
          <div className="hidden md:flex flex-wrap gap-1.5 mb-3">
            {(quickReplies.length > 0 ? quickReplies : interviewSuggestions).map((s) => (
              <button
                key={s}
